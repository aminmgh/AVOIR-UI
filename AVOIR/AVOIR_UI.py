...
import pdb
import time
from PIL import Image   # or just use st.image directly with a file/URL path
...
if __name__ == "__main__":
    ...
    with tab1:
        ...
        if selected_dataset_attrs:
            ...
            initial_spec_code = selected_spec_template.format(**selected_dataset_attrs)
            selected_spec_code = st.text_input(f"#### Generated Specification:", initial_spec_code)
        else:
        ...
        #     st.markdown("Oops! Looks like the chosen training data isn't compatible with the selected model. Select another configuration")
    with tab2:
        # Load the images (local file, BytesIO, URL, etc.)
        img1 = Image.open("plots/compas-northpointe-et.png")
        img2 = Image.open("plots/compas-propublica-et.png")

        # Create a 2-column layout
        col1, col2 = st.columns(2)

        with col1:
            st.markdown("**Compas-Northpointe:    E[recid=0|hrisk & r=C] / E[recid=0|hrisk & r=AA]**")
            st.image(img1, use_column_width=True)

        with col2:
            st.markdown("**Compas-Propublica:    E[hrisk|recid=0 & r=C] / E[hrisk|recid=0 & r=AA]**")
